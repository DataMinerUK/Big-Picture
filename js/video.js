...
/*    for (var j = 0; j < event.connections.length; j++) {
        var currenthtml = "<li id='" + event.connections[j].connectionId + "'>" +
                               ...
                               "<p style='text-align: right;'><input type='button' value='Request video' onclick='javascript:request_video(\"" + event.connections[i].connectionId + "\")' />&nbsp;<input type='button' value='Disconnect Video' onclick='javascript:disconnect_video(\"" + event.connections[j].connectionId + "\")' /></p>" +
                                "</li>";
...
function record_video() {
    hide('recordLink');
    show('stopLink');
    archive = session.createArchive(apiKey, "perSession", "BigPicture " + date.now);
